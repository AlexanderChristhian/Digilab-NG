...
const AssignmentForm = ({ isEditing = false }: AssignmentFormProps) => {
  ...
  const [classId, setClassId] = useState<number | null>(classIdFromQuery ? parseInt(classIdFromQuery) : null);
    ...
    const fetchData = async () => {
      try {
        ...
        if (isEditing && id) {
          ...
          const selectedClass = classesResponse.data.find((c: Class) => c.id === parseInt(classIdFromQuery));
          setSelectedClass(selectedClass || null);
        }
