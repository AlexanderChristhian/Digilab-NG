...
const ModuleForm = ({ isEditing = false }: ModuleFormProps) => {
  ...
  useEffect(() => {
    if (!isEditing) {
      ...
      if (classIdParam) {
        setClassId(parseInt(classIdParam, 10));
      }

      if (folderIdParam) {
        setFolderId(parseInt(folderIdParam, 10));
      }
    }
            ...
            {!isEditing && (
              <div className="mb-4">
                ...
                <select
                  id="class"
                  value={classId || ''}
                  onChange={(e) => setClassId(e.target.value ? parseInt(e.target.value, 10) : null)}
                  className="w-full px-3 py-2 border border-secondary-300 dark:border-dark-border rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-dark-text"
                  required
                >
          ...
          <div className="bg-white dark:bg-gradient-to-r dark:from-gray-800 dark:to-gray-700 rounded-lg shadow mb-6">
            ...
            <div className="p-4">
              {!previewMode ? (
                ...
                <div className="prose dark:prose-invert prose-primary max-w-none min-h-[300px] p-4 border border-secondary-200 dark:border-dark-border rounded-md text-secondary-700 dark:text-dark-text">
                  {content ? (
                    <ReactMarkdown
